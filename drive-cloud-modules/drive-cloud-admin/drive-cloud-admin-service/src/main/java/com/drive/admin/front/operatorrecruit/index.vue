<template>
    <div class="app-container">
        <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
                        ...
                        <el-input
                                v-model="queryParams.applyExplain"
                                placeholder="请输入申请说明"
                                clearable
                                size="small"
                                @keyup.enter.native="handleQuery"
        ...
        <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
                <el-button
                    ...
                    size="mini"
                    @click="handleAdd"
                    v-hasPermi="['admin:operatorRecruit:add']"
                >新增</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button
                    ...
                    :disabled="single"
                    @click="handleUpdate"
                    v-hasPermi="['admin:operatorRecruit:edit']"
                >修改</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button
                    ...
                    :disabled="multiple"
                    @click="handleDelete"
                    v-hasPermi="['admin:operatorRecruit:delete']"
                >删除</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button
                    ...
                    size="mini"
                    @click="handleExport"
                    v-hasPermi="['admin:operatorRecruit:export']"
                >导出</el-button>
            </el-col>
        </el-row>

        <el-table v-loading="loading" :data="operatorRecruitList" @selection-change="handleSelectionChange"
        ...
        >
            <el-table-column type="selection" width="55" align="center" />
                                                ...
                                                <el-table-column label="主键" align="center" prop="id" sortable="custom"/>
                                                                ...
                                                                <el-table-column label="姓名" align="center" prop="realName" sortable="custom"/>
                                                                ...
                                                                <el-table-column label="公司名称" align="center" prop="corporationName" sortable="custom"/>
                                                                <el-table-column label="申请说明" align="center" prop="applyExplain" sortable="custom"/>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getListPage() {
                this.loading = true;
                listPageOperatorRecruit(this.queryParams).then(response => {
                    this.operatorRecruitList = response.data.records;
            ...
            handleAdd() {
                this.reset();
                this.open = true;
                this.title = "添加运营商加盟申请";
            },
            /** 修改按钮操作 */
            handleUpdate(row) {
                this.reset();
                const id = row.id || this.ids
                getOperatorRecruit(id).then(response => {
                    this.form = response.data;
                    this.open = true;
                    this.title = "修改运营商加盟申请";
                });
            },
            /** 提交按钮 */
            submitForm: function() {
                this.$refs["form"].validate(valid => {
                    if (valid) {
                        if (this.form.id != undefined) {
                            updateOperatorRecruit(this.form).then(response => {
                            ...
                            addOperatorRecruit(this.form).then(response => {
                                if (response.code === 200) {
            ...
            handleExport() {
                ...
                this.$confirm('是否确认导出所有运营商加盟申请数据项?', "警告", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    type: "warning"
                }).then(function() {
                    return exportOperatorRecruit(queryParams);
                }).then(data => {
                    this.download(data, "运营商加盟申请");
                }).catch(function() {});
            }
        }
    };
