<template>
    <div class="app-container">
        <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
                        ...
                        <el-input
                                v-model="queryParams.evaluateTagId"
                                placeholder="请输入评价标签id"
                                clearable
                                size="small"
                                @keyup.enter.native="handleQuery"
                        />
                        ...
                        <el-input
                                v-model="queryParams.studentCoachAppraiseId"
                                placeholder="请输入学员教练评价表id"
                                clearable
                                size="small"
                                @keyup.enter.native="handleQuery"
                        />
        ...
        <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
                <el-button
                    ...
                    size="mini"
                    @click="handleAdd"
                    v-hasPermi="['admin:evaluateTagAppraise:add']"
                >新增</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button
                    ...
                    :disabled="single"
                    @click="handleUpdate"
                    v-hasPermi="['admin:evaluateTagAppraise:edit']"
                >修改</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button
                    ...
                    :disabled="multiple"
                    @click="handleDelete"
                    v-hasPermi="['admin:evaluateTagAppraise:delete']"
                >删除</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button
                    ...
                    size="mini"
                    @click="handleExport"
                    v-hasPermi="['admin:evaluateTagAppraise:export']"
                >导出</el-button>
            </el-col>
        </el-row>

        <el-table v-loading="loading" :data="evaluateTagAppraiseList" @selection-change="handleSelectionChange"
        ...
        >
            <el-table-column type="selection" width="55" align="center" />
                                                ...
                                                <el-table-column label="id" align="center" prop="id" sortable="custom"/>
                                                                <el-table-column label="评价标签id" align="center" prop="evaluateTagId" sortable="custom"/>
                                                                <el-table-column label="学员教练评价表id" align="center" prop="studentCoachAppraiseId" sortable="custom"/>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getListPage() {
                this.loading = true;
                listPageEvaluateTagAppraise(this.queryParams).then(response => {
                    this.evaluateTagAppraiseList = response.data.records;
            ...
            handleAdd() {
                this.reset();
                this.open = true;
                this.title = "添加学员教练评价明细表";
            },
            /** 修改按钮操作 */
            handleUpdate(row) {
                this.reset();
                const id = row.id || this.ids
                getEvaluateTagAppraise(id).then(response => {
                    this.form = response.data;
                    this.open = true;
                    this.title = "修改学员教练评价明细表";
                });
            },
            /** 提交按钮 */
            submitForm: function() {
                this.$refs["form"].validate(valid => {
                    if (valid) {
                        if (this.form.id != undefined) {
                            updateEvaluateTagAppraise(this.form).then(response => {
                            ...
                            addEvaluateTagAppraise(this.form).then(response => {
                                if (response.code === 200) {
            ...
            handleDelete(row) {
                ...
                this.$confirm('是否确认删除?', "警告", {
                    ...
                    return delEvaluateTagAppraise(ids);
                }).then(() => {
                    this.getListPage();
            ...
            handleExport() {
                ...
                this.$confirm('是否确认导出所有学员教练评价明细表数据项?', "警告", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    type: "warning"
                }).then(function() {
                    return exportEvaluateTagAppraise(queryParams);
                }).then(data => {
                    this.download(data, "学员教练评价明细表");
                }).catch(function() {});
            }
        }
    };
