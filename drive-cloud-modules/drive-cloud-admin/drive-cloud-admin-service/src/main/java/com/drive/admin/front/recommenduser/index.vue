...
<script>
import { listRecommendUser, getRecommendUser, delRecommendUser, addRecommendUser, updateRecommendUser, exportRecommendUser } from "@/api/admin/recommendUser";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRecommendUser(this.queryParams).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有推广商信息数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
