    ...
    export default {
        ...
        created() {
            ...
            if (!Utils.isEmptyStr(this.id)) {
                this.getCoachHourSettingData();
            }
        },
        methods: {
            // 通过ID 查询数据
            getCoachHourSettingData(){
                console.log('getBanner请求参数{}',this.id);
                getCoachHourSetting(this.id).then(response => {
            ...
            publishData: function() {
                this.$refs["form"].validate(valid => {
                    if (valid) {
                        if (Utils.isEmptyStr(this.id)) {
                            // 添加接口
                            this.addCoachHourSettingData(this.form);
                        }else{
                            // 修改接口
                            this.updateCoachHourSettingData(this.form)
                        }
                    }
                });
            },
            // 添加数据方法
            addCoachHourSettingData(data){
                addCoachHourSetting(data).then(response => {
            ...
            // 修改数据方法
            updateCoachHourSettingData(data){
                updateCoachHourSetting(data).then(response => {
                    console.log("接口请求数据：", response);
                    let subCode = response.subCode;
                    if (subCode == 'SUCCESS'){
                        this.$message({
