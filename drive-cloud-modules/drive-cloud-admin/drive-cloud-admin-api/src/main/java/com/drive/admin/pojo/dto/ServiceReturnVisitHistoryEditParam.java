 ...
 */
@Data
public class ServiceReturnVisitHistoryEditParam {
    ...
    @NotBlank(message = "回访类型（1-学车报名；2-考试报名；3-常规训练；4-考试训练）不能为空")
    ...
    // 是否完结（是，否）
    @NotBlank(message = "是否完结（是，否）不能为空")
    @ApiModelProperty(value = "是否完结（是，否）")
    private String isEnd;

    // 科目类型(1-科目一，2-科目三，3-科目三，4-科目四)
    @NotBlank(message = "科目类型(1-科目一，2-科目三，3-科目三，4-科目四)不能为空")
